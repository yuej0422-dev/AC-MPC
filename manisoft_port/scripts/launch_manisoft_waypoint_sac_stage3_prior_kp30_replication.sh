...
eval_root="$project_root/runs/manisoft_waypoint_sac_stage3_prior_heldout_eval_20260823"
variant="prior40_kp30"

test -f "$model"
test -f "$vecnorm"
test -d "$eval_root/pure_sac"
test ! -e "$eval_root/$variant"
mkdir -p "$eval_root/$variant"

cd "$project_root"
for chunk in $(seq 0 3); do
  seed=$((867000 + chunk * 48))
  nohup setsid env \
    ...
    "$python_bin" -u scripts/evaluate_manisoft_waypoint_sac.py \
      ...
      --output "$eval_root/$variant/chunk_$chunk.npz" --episodes 48 \
      ...
      --cartesian-prior-proportional-gain 30 \
      --cartesian-prior-feedforward-scale 1 \
      --successful-only --device cpu --seed "$seed" \
      >"$eval_root/$variant/chunk_$chunk.log" 2>&1 </dev/null &
  echo "$!" >"$eval_root/$variant/chunk_$chunk.pid"
done

echo "launched four kp30 replication workers under $eval_root/$variant"
