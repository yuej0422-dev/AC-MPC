...
source_step="799856"
output="${1:-$project_root/runs/manisoft_ground015_stage1b_always_residual_pilot_20260825}"
...
exec "$python_bin" -u scripts/train_manisoft_waypoint_sac.py \
  ...
  --residual-stall-activation-steps 0 \
  --residual-stall-ramp-steps 1 \
  --zero-init-actor \
  --actor-anchor-coef 50 \
  --source-policy-warmup \
  --actor-learning-delay-steps 7500 \
