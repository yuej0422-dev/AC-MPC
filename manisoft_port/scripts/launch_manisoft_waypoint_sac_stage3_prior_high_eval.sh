...
launch_variant() {
  ...
  test ! -e "$eval_root/$name"
  mkdir -p "$eval_root/$name"
  for chunk in $(seq 0 3); do
    local seed=$((867000 + chunk * 48))
    nohup setsid env \
...
cd "$project_root"
launch_variant prior60 0.60
launch_variant prior80 0.80

echo "launched eight high-weight prior workers under $eval_root"
