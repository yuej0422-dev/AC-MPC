...
output="$project_root/runs/manisoft_waypoint_arch_base070_xyz_stage2_frozen_residual_moderate_v2_20260824"
...
exec "$python_bin" -u scripts/train_manisoft_waypoint_sac.py \
  ...
  --frozen-base-model "$source_run/checkpoints/sac_table_waypoint_polyline_24996_steps.zip" \
  --frozen-base-vec-normalize "$source_run/checkpoints/sac_table_waypoint_polyline_vecnormalize_24996_steps.pkl" \
  --residual-action-scale 0.050 \
  --residual-action-penalty-scale 0.050 \
  --zero-init-actor \
  --actor-anchor-coef 100 \
  --source-policy-warmup \
  --actor-learning-delay-steps 15000 \
