...
source_run="$project_root/runs/manisoft_waypoint_arch_base070_xy_tolerant_pilot_r050_v1_20260824"
output="$project_root/runs/manisoft_waypoint_arch_base070_xy_stall_gate200_eval_v1_20260824"
...
exec "$python_bin" -u scripts/train_manisoft_waypoint_sac.py \
  ...
  --eval-episodes 12 --eval-num-envs 3 --eval-panel-seed 965000 \
  ...
  --resume "$source_run/checkpoints/sac_table_waypoint_polyline_19992_steps.zip" \
  --vec-normalize "$source_run/checkpoints/sac_table_waypoint_polyline_vecnormalize_19992_steps.pkl" \
  ...
  --residual-action-scale 0.50 --residual-action-penalty-scale 0.005 \
  --residual-stall-activation-steps 200 --residual-stall-ramp-steps 300 \
  --actor-anchor-coef 2 --source-policy-warmup --freeze-vec-normalize \
  --device cuda --seed 20268123
