...
for index in "${!variants[@]}"; do
  ...
  model="$output/checkpoints/sac_table_long_waypoints_10000_steps.zip"
  vecnorm="$output/checkpoints/sac_table_long_waypoints_vecnormalize_10000_steps.pkl"
  test -f "$model"
  test -f "$vecnorm"
  nohup setsid env \
    ...
    "$python_bin" -u scripts/train_manisoft_waypoint_sac.py \
      ...
      --scenario "$scenario" --output "$output" --allow-existing-output \
